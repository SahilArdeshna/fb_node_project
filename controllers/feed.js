...
exports.getFeeds = async (req, res, next) => {
  ...
  const feeds = await Feed.find({ user: { $in: userWithFriendsIds } })
    .populate("user")
    .sort({ createdAt: -1 });
  ...
  for (let feed of feeds) {
    for (let comment of feed.comments) {
      ...
      commentUser.profileImage =
        commentUser.profileImage || "images/profile-images/default-profile.png";
...
exports.postFeed = async (req, res, next) => {
  ...
  try {
    ...
    if (checkVideoType(file)) {
      ...
      const video = fs.createWriteStream(
        `images/${path}/${time}-${file.originalname}`
      );
      if (!video) {
